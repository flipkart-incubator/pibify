...
## Flow
1. Clients configure a maven plugin which scans the source of the project during the build phase and collects all pojos which have the `@Pibify` annotation.
2. This plugin generates the `Handler` and `PibifyHandlerCache` for the configured module and places them at a suitable place in the package(jar)
3. Clients use the `PibifyHandlerCache` to get a reference to a `Handler` and call the serialize/deserialize method on it.


# Usage
1. Configure the model mvn module by adding the below plugin config
```xml
<plugin>
...
2. As part of the `package` mvn goal, the code will be generated
